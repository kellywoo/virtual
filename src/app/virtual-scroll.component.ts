...
export class VirtualScrollComponent implements AfterViewInit, OnInit {
  ...
  private updatePosition() {
    if (this.locationKey) {
      const pp = this.keySequence.indexOf(this.locationKey);
      if (pp > -1) {
        return;
      }
    }
    if (this.direction === 'top') {
      this.locationKey = this.keySequence[0];
    } else {
      this.locationKey = this.keySequence[0];
    }
  }
  ...
  private updateContainer(container: HTMLElement) {
    ...
    this.cacheContainerSize(container);
  ...
  private cacheContainerSize(wrapper: HTMLElement) {
